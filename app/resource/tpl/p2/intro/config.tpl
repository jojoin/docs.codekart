...
<pre><code>//load.config_path = '';
//load.config_path = 'production';
//load.config_path = 'test';
load.config_path = 'development';</code></pre>

<p>在此处修改你的当前的配置路径，如果<var>config_path</var>为空，则使用<var>app/config/</var>目录而不是自定义的目录。</p>
...
<p>如同普通的<dfn>Node.js</dfn>模块一样，配置文件在底层通过<code>require(model);</code>函数加载，可以在配置文件中填写任意js代码，只需要加入<code>exports</code>对外接口即可。</p>
<p></p>
