...
<p>幸好，CodeTank 可以为你把切分成模块的 js、css 和 html 组合起来，压缩并自动添加引用。我们将通过一个示例来详细讲解用法。</p>

<p>在<dfn>framework/view</dfn>文件夹下面包含四个文件<var>html.js</var>,<var>home.js</var>,<var>404.js</var>和<var>error.js</var>。
  这是框架自带的基础页面，你不需要修改它。如果需要自定义这几个页面，
  你可以在<dfn>app/view</dfn>下新建同名的文件，并按照配置自定义。框架会优先调用<dfn>app/view</dfn>下面的，
  而<dfn>framework/view</dfn>下的就会被“重载”。</p>

<p>它们就是四个基础web页面，其中其余三个页面都是从<var>html.js</var><a href="/p/core_web_inherit">继承</a>过来。</p>
...
<p><kdb>注意：</kdb>tpl ,  tpl_pre , less , csslib , js , jslib 配置变量可以是一个路径文件名称，并且省略后缀名。</p>
